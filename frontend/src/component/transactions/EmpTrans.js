...
class EmpTrans extends React.Component {
    ...
    */
    ...
    nameInputHandlerForSearch = (e) => {
        ...
        if (e.key === 'Enter') {
            this.props.getEmpExp(`employee_experince.NATIONAL_ID_CARD_NO = (SELECT NATIONAL_ID_CARD_NO FROM employee WHERE NAME_ARABIC = "${this.props.nameOrId}")`)
            this.props.getEmpTrans("", e.target.value)
        }
    }
    ...
    handleExpTime = (startDate, empexp) => {
        ...
        if (empexp.length > 0) {
            militerExpStart = empexp.filter(el => el.EXP_TYP_CODE === 1)[0].START_DATE
            militerExpEnd = empexp.filter(el => el.EXP_TYP_CODE === 1)[0].END_DATE
            outerSectorStart = empexp.filter(el => el.EXP_TYP_CODE === 3).START_DATE
            outerSectorEnd = empexp.filter(el => el.EXP_TYP_CODE === 3).END_DATE
            innerSectorStart = empexp.filter(el => el.EXP_TYP_CODE === 4).START_DATE
            innerSectorEnd = empexp.filter(el => el.EXP_TYP_CODE === 4).END_DATE
        ...
        console.log(diffWith, diffWithout, diffMiliter);

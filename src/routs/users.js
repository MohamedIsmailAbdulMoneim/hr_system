...
function handleRegister(req, res, next) {
    ...
    db.query(`SELECT * FROM users WHERE username = "${uname}"`, function (err, rows) {
        if ((!uname || uname == 'undefined') || (!pw || pw == 'undefined')) {
            ...
            var insertQuery = "INSERT INTO users ( username, salt, hash ) values (?,?,?)";
            db.query(insertQuery, [newUserMysql.username, newUserMysql.salt, newUserMysql.hash], function (err, rows) {
